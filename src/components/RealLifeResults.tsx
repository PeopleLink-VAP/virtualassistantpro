...
const RealLifeResults = () => {
  ...
  return (
    <section className="py-16 md:py-20 bg-gradient-to-br from-sunflower/5 to-navy/5 relative overflow-hidden">
      {/* Decorative elements */}
      <div className="absolute top-10 right-4 md:right-10 w-20 h-20 md:w-32 md:h-32 bg-sunflower/10 rounded-full animate-spin-slow"></div>
      <div className="absolute bottom-20 left-4 md:left-20 w-16 h-16 md:w-20 md:h-20 bg-leafGreen/15 rounded-full animate-float"></div>
      
      <div className="container mx-auto px-4">
        {/* Header Section */}
        <div className="text-center mb-12 md:mb-16">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-['Roboto_Condensed'] text-navy mb-4">
            Kết Quả <span className="text-sunflower font-condensed">Thực Tế</span>
          </h2>
          <p className="text-base md:text-lg text-navy/70 max-w-2xl mx-auto px-4">
        ...
        <div className="relative max-w-6xl mx-auto">
            ...
            {testimonials.map((testimonial, index) => (
              ...
              >
                ...
                {hoveredTestimonial === testimonial.id && (
                  <div className="hidden md:block absolute bottom-20 left-1/2 transform -translate-x-1/2 w-80 bg-white/95 backdrop-blur-sm rounded-xl p-6 shadow-2xl z-20 animate-fade-in">
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="min-w-0">
                        <h4 className="font-semibold text-navy truncate">{testimonial.name}</h4>
            ...
            <div className="md:hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-xl p-6 max-w-sm w-full max-h-[80vh] overflow-y-auto">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full overflow-hidden flex-shrink-0">
                      <img 
                        src={testimonials.find(t => t.id === selectedTestimonial)?.avatar} 
                        alt={testimonials.find(t => t.id === selectedTestimonial)?.name} 
                        className="w-full h-full object-cover" 
                      />
                    </div>
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-navy/70">
                        {testimonials.find(t => t.id === selectedTestimonial)?.role}
                      </p>
                    </div>
                  </div>
                  <button 
                    onClick={() => setSelectedTestimonial(null)}
                    className="text-navy/50 hover:text-navy text-xl font-bold"
                  >
                    ×
                  </button>
                </div>
                ...
                <div className="flex gap-1 mb-4">
                  {[...Array(testimonials.find(t => t.id === selectedTestimonial)?.rating || 5)].map((_, i) => (
                    <Star key={i} size={16} className="fill-sunflower text-sunflower" />
                  ))}
                </div>
          ...
          {/* Summary Stats */}
          <div className="bg-white/80 backdrop-blur-sm rounded-xl p-6 md:p-8 shadow-lg max-w-4xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 text-center">
              <div className="space-y-2">
                <div className="text-2xl md:text-3xl font-bold text-sunflower">100%</div>
                <p className="text-navy/80 text-sm md:text-base leading-relaxed">
                  học viên được hướng dẫn<br className="hidden md:block" />
                  từ A-Z về nghề VA
                </p>
              </div>
              <div className="space-y-2">
                <div className="text-2xl md:text-3xl font-bold text-leafGreen">70 - 85%</div>
                <p className="text-navy/80 text-sm md:text-base leading-relaxed">
                  học viên có khách hàng<br className="hidden md:block" />
                  (tối thiểu 4 tháng quyết liệt làm)
                </p>
              </div>
              <div className="space-y-2">
                <div className="text-2xl md:text-3xl font-bold text-navy">4.9/5</div>
                <p className="text-navy/80 text-sm md:text-base leading-relaxed">
                  Đánh giá khóa học từ 150 học viên
                </p>
              </div>
            </div>
          </div>
